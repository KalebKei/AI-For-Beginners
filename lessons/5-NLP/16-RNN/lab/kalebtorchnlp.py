...
#         self.texts = texts.reset_index(drop=True)  # Ensure index alignment
#         self.labels = labels.reset_index(drop=True)  # Ensure index alignment
...
def load_dataset(datasetDir, target_column, classes, ngrams=1,min_freq=1):
    global vocab, tokenizer
    print("Loading dataset...")

    # Kaleb changed out code  
    df = pd.read_csv(datasetDir)
    train_df, test_df = preprocess(df)

    train_dataset = CSVDataset(train_df, target_column)
    test_dataset = CSVDataset(test_df, target_column)
